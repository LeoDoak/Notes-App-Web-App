...
def create_database():
    ...
    cursor.execute(
    "CREATE TABLE IF NOT EXISTS user(user_id INTEGER, username TEXT, password TEXT,email TEXT)")
    cursor.execute(
    "SELECT username, password FROM user where user_id = 0 and email = 'admin@uncw.edu'")
    row = cursor.fetchall()
